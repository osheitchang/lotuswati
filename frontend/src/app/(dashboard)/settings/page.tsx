...
export default function SettingsPage() {
  ...
  const handleUpdateAgentRole = async (id: string, role: string) => {
    try {
      await teamApi.updateAgent(id, { role })
      updateAgent({ id, role: role as any })
    } catch {
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-2 mt-1">
                        ...
                        <Button
                          ...
                          size="icon"
                          className="h-9 w-9 flex-shrink-0"
                          onClick={() => copyToClipboard(team.webhookVerifyToken!)}
                        >
                          <Copy className="w-4 h-4" />
                        </Button>
                      </div>
                    ...
                    agents.map((agent) => (
                      <div key={agent.id} className="flex items-center gap-3 px-5 py-3">
                        ...
                        <div className="flex items-center gap-2">
                          <Select
                            value={agent.role}
                            onValueChange={(v) => handleUpdateAgentRole(agent.id, v)}
                          >
        ...
        <DialogContent className="max-w-sm">
                ...
                {LABEL_COLORS.map((color) => (
                  <button
                    ...
                    className={cn(
                      ...
                      labelForm.color === color ? 'border-gray-800 scale-110' : 'border-transparent'
                    )}
          ...
          <DialogFooter>
            ...
            <Button onClick={handleSaveLabel} disabled={isSavingLabel}>
              {isSavingLabel ? 'Saving...' : editingLabel ? 'Update' : 'Create'}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={showCannedDialog} onOpenChange={setShowCannedDialog}>
        <DialogContent className="max-w-md">
          ...
          <DialogFooter>
            ...
            <Button onClick={handleSaveCanned} disabled={isSavingCanned}>
              {isSavingCanned ? 'Saving...' : editingCanned ? 'Update' : 'Create'}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
